...
public: // Function declarations
  ...
  double get_temp(); //returns actual temperature value
  double get_rho(); //returns density of the system
  double get_r_cut(); //returns cutoff radius for pair interactions
  double get_acceptance(); //returns the acceptance of Metropolis algorithm
  int get_sim_type(); //returns simulatioin type
  ...
  void initialize_equilibrated(string phase); //initialize system properties with equilibrated temperature

  void set_temp(double temp); //changing temperature and beta of the system
  void set_delta(); //setting step lenght in order to have 50% acceptance
