...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ofstream coutf;
  coutf.open(_path_out+"/output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        ofstream coutk(_path_out+"/kinetic_energy.dat");
        ...
        ofstream coutte(_path_out+"/temperature.dat");
        ...
        ofstream coutpr(_path_out+"/pressure.dat");
        ...
        ofstream coutgr(_path_out+"/gofr.dat");
        ...
        ofstream coutf;
        coutf.open(_path_out+"/output.dat",ios::app);
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(_path_out+"/CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.open(_path_out+"/CONFIG/config.spin");
...
void System :: read_configuration(){
  ...
  if(_restart and _sim_type > 1){
    int spin;
    cinf.open(_path_out+"/CONFIG/config.spin");
    for(int i=0; i<_npart; i++){
      cinf >> spin;
...
void System :: measure(){ // Measure properties
      ...
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr<_halfside.min()){
          for(int k=0; k<_n_bins; k++){
            if(dr>k*_bin_size and dr<(k+1)*_bin_size){
              _measurement(_index_gofr+k)+=2/(_rho*_npart*(4./3.)*M_PI*(pow((k+1)*_bin_size,3)-pow(k*_bin_size,3)));
            }
          }
        }
  ...
  _measurement(_index_pressure) = _rho*_measurement(_index_temp)+(48.*pressure_temp/(3.*double(_npart))+_ptail)/(_volume);
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if(_measure_magnet){
    if(_H==0){
      _measurement(_index_magnet)=0.0;
    }
    else{
      double sum_s_i=0;
      for(int i=0; i<_npart; i++){
  ...
  if(_measure_cv){
    if(_H!=0){
      ...
      for (int i=0; i<_npart; i++){
        s_i = double(_particle(i).getspin());
        s_j = double(_particle(this->pbc(i+1)).getspin());
        tenergy_temp1 += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
        tenergy_temp2 += pow((- _J * s_i * s_j - 0.5 * _H * (s_i + s_j)),2);
      }
      tenergy_temp1 /= double(_npart);
      tenergy_temp2 /= double(_npart);
      _measurement(_index_cv)=pow(_beta,2)*(tenergy_temp2-pow(tenergy_temp1,2));
    }
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi){
...
void System :: averages(int blk){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open(_path_out+"/potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(_path_out+"/kinetic_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(_path_out+"/temperature.dat",ios::app);
  ...
  if (_measure_pressure){
    coutf.open(_path_out+"/pressure.dat",ios::app);
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if(_measure_gofr){
    coutf.open(_path_out+"/gofr.dat",ios::app);
    for(int k=0; k<_n_bins; k++){
    ...
    coutf.close();
    if(blk==100){
      ofstream coutg(_path_out+"/gofr_fin.dat");
      coutg << "#    DISTANCE:      AVERAGE_GOFR:       ERROR:" << endl;
      for(int k=0; k<_n_bins; k++){
        coutg << setw(12) << k*_bin_size
            << setw(12) << _global_av(_index_gofr+k)/double(blk)
            << setw(18) << this->error(_global_av(_index_gofr+k), _global_av2(_index_gofr+k), blk) << endl;
      }
      coutf.close();
    }
  }
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open(_path_out+"/acceptance.dat",ios::app);
...
double System::get_acceptance(){
  return double(double(_naccepted)/_nattempts);
}
...
void System :: initialize_equilibrated(string phase){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> temp_init; //saving real initial temperature
      _temp=temp_init;
      _beta=1./_temp;
      coutf << "TEMPERATURE= " << temp_init << endl;
    } else if( property == "NPART" ){
      input >> _npart;
      _fx.resize(_npart);
      _fy.resize(_npart);
      ...
      coutf << "STARTING TEMPERATURE= " << _temp << endl;
    } else if( property == "ENDINPUT" ){
      coutf << "Reading input completed!" << endl;
      break;
    } else cerr << "PROBLEM: unknown input" << endl;
...
void System::set_delta(){
  for(int i=0;i<100;i++){
      this->step();
  }

  cout << this->get_acceptance()<< endl;;

  while(this->get_acceptance()>0.501 or this->get_acceptance()<0.499){

    cout << "Acceptance: "<<this->get_acceptance() << endl;

    if(this->get_acceptance()>0.501){
      _delta=_delta+0.0005;
      cout << "increasing step lenght by 0.0005" << endl;
    }

    else{
      _delta=_delta-0.0005;
      cout << "decreasing step lenght by 0.0005" << endl;
    }
  ...
  cout << "Final step lenght: " << _delta << endl;
  _naccepted=0;
  _nattempts=0;
}
