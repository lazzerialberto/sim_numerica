...
void element::generate_initial_config(Random &_rnd,bool circle){
    ...
    else{ //generate initial configuration inside a 1x1 square
...
void Population::initial_pop(Random &_rnd,bool circle){
    ...
    _pop[0].print_config(path_name+"init_conf_circle.dat");
...
TSP::TSP(){
    ...
    while(!fin.eof()){
        ...
        if(property=="Configuration"){
            ...
            if(disp=="circle"){
                _circle=true;
                path_name="./OUTPUT/CIRCLE/";
            }
            else{
                _circle=false;
                path_name="./OUTPUT/SQUARE/";
            }
        }
    ...
    ofstream out2(path_name+"best_loss.dat");
    ofstream out3(path_name+"half_loss.dat");
...
//copy constructor and operator= for element class
/*element::element(const element & e){

    _r.resize(e.get_n_cities());
    _labels.resize(e.get_n_cities());

    for(int i=0; i<_ncities; i++){
        _labels[i]=e.get_label(i);
        _r[i]=e.get_pos(i);
    }
    _dist=e.get_dist();
    _ncities=e.get_n_cities();
}
...
//copy constructor and operator= for Population class
/*Population::Population(const Population & pop){

    element e(pop.get_n_cities());
    _pop.resize(pop.get_n_individuals(),e);

    for(int i=0; i<pop.get_n_individuals(); i++){
        _pop[i]=pop.get_element(i);
    }
    
    _ncities=pop.get_n_cities();
    _nindividuals=pop.get_n_individuals();
    _ordered=false;
}
...
/*bool TSP::check(int j, std::vector<int> vec){
    bool ok=true;
...
Population TSP::Crossover(Random rnd, Population pop){

    Population new_pop(rnd,pop.get_n_individuals(),pop.get_n_cities());

    std::vector<int> wout_repl;

    for(int i=0; i<pop.get_n_individuals(); i++){

        if(check(i,wout_repl)){

            int appo=static_cast<int>(rnd.Rannyu(i+1,pop.get_n_individuals()-1));
            bool appo2=check(appo,wout_repl);

            while(!appo2){
                appo=static_cast<int>(rnd.Rannyu(i+1,pop.get_n_individuals()-1));
                appo2=check(appo,wout_repl);
            }

            if(rnd.Rannyu()<=_p_cross){

                element father(pop.get_element(i));
                element mother(pop.get_element(appo));
                std::vector<element> offsprings;

                offsprings=father.crossover(father,mother);
                new_pop.Set_element(i,offsprings[0]);
                new_pop.Set_element(appo,offsprings[1]);

            }
        
            else{
                new_pop.Set_element(i,pop.get_element(i));
                new_pop.Set_element(i,pop.get_element(appo));
            }
            wout_repl.push_back(appo);
        }
    }
